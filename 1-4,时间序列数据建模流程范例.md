...
```python
#表示将matplotlib图表嵌入到Notebook
%matplotlib inline
%config InlineBackend.figure_format = 'svg'#在默认设置的matplotlib中图片分辨率不是很高，可以通过设置矢量图的方式来提高图片显示质量

df = pd.read_csv("./data/covid-19.csv",sep = "\t")#读取csv文件 sep指定分隔符
df.plot(x = "date",y = ["confirmed_num","cured_num","dead_num"],figsize=(10,6))#设置图形的长宽
plt.xticks(rotation=60)#设置x轴坐标旋转60度
...
```python
dfdata = df.set_index("date")#设置data的索引
dfdiff = dfdata.diff(periods=1).dropna()#后一天的数据减前一天的 得到新增数   dropna()滤除缺失数据
dfdiff = dfdiff.reset_index("date")#reset_index可以还原索引，重新变为默认的整型索引

dfdiff.plot(x = "date",y = ["confirmed_num","cured_num","dead_num"],figsize=(10,6))
plt.xticks(rotation=60)
dfdiff = dfdiff.drop("date",axis = 1).astype("float32")#删除日期这一列不是整型的数据
...
def batch_dataset(dataset):
    dataset_batched = dataset.batch(WINDOW_SIZE,drop_remainder=True)#从数据集取出8个数据 参数drop_remaindar
                                                                   # 用于标示是否对于最后一个batch如果数据量达不到batch_size时保留还是抛弃
    return dataset_batched

ds_data = tf.data.Dataset.from_tensor_slices(tf.constant(dfdiff.values,dtype = tf.float32))\#把某日前八天的数据传入数据管道
   .window(WINDOW_SIZE,shift=1).flat_map(batch_dataset)

ds_label = tf.data.Dataset.from_tensor_slices(
    tf.constant(dfdiff.values[WINDOW_SIZE:],dtype = tf.float32))#某日的标签数据

#数据较小，可以将全部训练数据放入到一个batch中，提升性能
ds_train = tf.data.Dataset.zip((ds_data,ds_label)).batch(38).cache()#将对象中对应的元素打包成一个个元组 共38个batch  cache()缓存
...
#return_sequences:在输出序列中，返回单个 hidden state值还是返回全部time step 的 hidden state值。 False 返回单个， true 返回全部。
...
tb_callback = tf.keras.callbacks.TensorBoard(logdir, histogram_freq=1)#对于模型中各个层计算激活值和模型权重直方图的频率(训练轮数中)
...
def plot_metric(history, metric):
    ...
    plt.plot(epochs, train_metrics, 'bo--')#  # 蓝色圆点实线
...
dfresult = dfdiff[["confirmed_num","cured_num","dead_num"]].copy()#copy() 函数返回一个字典的浅复制
dfresult.tail()#默认是取dataframe中的最后五行
```
...
for i in range(100):
    arr_predict = model.predict(tf.constant(tf.expand_dims(dfresult.values[-38:,:],axis = 0)))#tf.expand_dims（）扩充维度

    dfpredict = pd.DataFrame(tf.cast(tf.floor(arr_predict),tf.float32).numpy(),#创建一个DataFrame对象 columns数据列
                columns = dfresult.columns)
    dfresult = dfresult.append(dfpredict,ignore_index=True)
```

```python
dfresult.query("confirmed_num==0").head()#query（）挑选数据  head()读取前五行数据
...
```python
model.save('./data/tf_model_savedmodel', save_format="tf")#将整个模型保存下来,以后直接载入模型与训练数据即可开始训练,不用再定义网络和编译模型

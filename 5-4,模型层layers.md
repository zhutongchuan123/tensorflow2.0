...
class Linear(layers.Layer):
    #__init__()方法是所谓的对象bai的“构造函数”，负责在对象初始化时进行一系列的构建操作
    ...
    def build(self, input_shape): 
        self.w = self.add_weight("w",shape=(input_shape[-1], self.units),#调用add_weight来做权重设置
                                 initializer='random_normal',#生成正态分布的随机数

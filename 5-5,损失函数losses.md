...
model.add(layers.Dense(64, input_dim=64,
                kernel_regularizer=regularizers.l2(0.01), #对该层中的权值进行正则化，亦即对权值进行限制
                activity_regularizer=regularizers.l1(0.01),#对该层的输出进行正则化
...
def focal_loss(gamma=2., alpha=0.75):
    ...
    def focal_loss_fixed(y_true, y_pred):
        ...
        loss = tf.reduce_sum(alpha_factor * modulating_factor * bce,axis = -1 )#reduce_sum( ) 是求和函数
                                                                               #可以通过调整 axis =0,1 的维度来控制求和维度。
        return loss

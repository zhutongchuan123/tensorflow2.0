...
def ks(y_true,y_pred):
    y_true = tf.reshape(y_true,(-1,))#转化成一行，行数未知
    y_pred = tf.reshape(y_pred,(-1,))
    length = tf.shape(y_true)[0]#返回y_true【0】的长度
    t = tf.math.top_k(y_pred,k = length,sorted = False)#这个函数的作用是返回 input 中每行最大的 k 个数
    y_pred_sorted = tf.gather(y_pred,t.indices)#将张量中对应索引的向量提取出来
    y_true_sorted = tf.gather(y_true,t.indices)
    cum_positive_ratio = tf.truediv(
        tf.cumsum(y_true_sorted),tf.reduce_sum(y_true_sorted))#TPR=TP/(TP+FN)
    cum_negative_ratio = tf.truediv(
        tf.cumsum(1 - y_true_sorted),tf.reduce_sum(1 - y_true_sorted))# FPR = FP/(FP+TN) 
    ks_value = tf.reduce_max(tf.abs(cum_positive_ratio - cum_negative_ratio)) # KS=max(TPR-FPR).
    return ks_value
```
...
class KS(metrics.Metric):
    ...
    @tf.function
    def update_state(self,y_true,y_pred):#编写update_state方法在每个batch后更新相关中间变量的状态
    ...
    @tf.function
    def result(self):#编写result方法输出最终指标结果
        cum_positive_ratio = tf.truediv(

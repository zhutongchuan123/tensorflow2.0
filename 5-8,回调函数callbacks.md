...
import json
json_log = open('./data/keras_log.json', mode='wt', buffering=1)#mode 决定了打开文件的模式 如果 buffering 的值取 1，访问文件时会寄存行
json_logging_callback = callbacks.LambdaCallback(
    on_epoch_end=lambda epoch, logs: json_log.write(#on_epoch_begin  要求两个位置型的参数： epoch, logs  在每轮结束时被调用
        json.dumps(dict(epoch = epoch,**logs)) + '\n'),#dict(iterable, **kwarg)  iterable -- 可迭代对象。   **kwargs -- 关键字
                                                       #json.dumps()用于将字典形式的数据转化为字符串
    on_train_end=lambda logs: json_log.close()#on_train_end 要求一个位置型的参数： logs    在模型训练结束时被调用。
)
...
class LearningRateScheduler(callbacks.Callback):
    ...
    def __init__(self, schedule, verbose=0):
        ...
        self.schedule = schedule#python中有一个轻量级的定时任务调度的库：schedule
        self.verbose = verbose#verbose = 0 为不在标准输出流输出日志信息

    def on_epoch_begin(self, epoch, logs=None):
        if not hasattr(self.model.optimizer, 'lr'):#hasattr()函数用于判断对象是否包含对应的属性。
        ...
        if not isinstance(lr, (tf.Tensor, float, np.float32, np.float64)):#isinstance函数可以对参数类型进行判断

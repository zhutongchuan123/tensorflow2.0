...
#镜像策略，该策略通常用于在一台具有多个GPU的机器上的训练

with strategy.scope(): #范围
    model = create_model()#创建模型
    model.summary()#输出模型各层的参数情况
    model = compile_model(model)#配置训练模型
    
history = model.fit(ds_train,validation_data = ds_test,epochs = 10)  #内置fit方法
```
